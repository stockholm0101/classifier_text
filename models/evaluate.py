...
def eval(model, test_data, test_label, thresholds=0.5, num_classes=2, pr_figure_path=None, pred_save_path=None):
    ...
    #test_label_ = test_label[test_label!=0]
    #label_pred_ = label_pred[test_label!=0]
    print(classification_report(test_label, label_pred, labels = np.arange(1,25)))

    save(label_pred, pred_save_path)
    return label_pred


def eval_deep(model, test_data, test_label, thresholds=0.5, num_classes=2, pr_figure_path=None, pred_save_path=None):
    #print('{0}, val mean acc:{1}'.format(model.__str__(), model.score(test_data, test_label)))
    if num_classes == 2:
        # binary classification
        label_pred_probas = model.predict_proba(test_data)[:, 1]
